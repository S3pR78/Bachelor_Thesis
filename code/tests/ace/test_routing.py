...
def test_safe_name_normalizes_model_ids() -> None:
    assert (
        safe_name("Qwen/Qwen2.5-Coder-7B-Instruct")
...
def test_resolve_prefers_model_specific_playbook(tmp_path: Path) -> None:
    ...
    shared = shared_dir / "nlp4re_pgmr_lite_playbook.json"
...
def test_resolve_falls_back_to_legacy_root_playbook(tmp_path: Path) -> None:
    legacy = tmp_path / "nlp4re_pgmr_lite_playbook.json"
