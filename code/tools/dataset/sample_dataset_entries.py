...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sample dataset entries from a JSON array."
    )
...
def main() -> int:
    ...
    if args.count > len(items):
        raise ValueError(
            f"Requested {args.count} items, but dataset only contains {len(items)} entries."
        )

    rng = random.Random(args.seed)
    sampled_items = rng.sample(items, args.count)

    save_json_file(output_path, sampled_items, overwrite=args.overwrite)

    print(f"Saved {len(sampled_items)} sampled entries to: {output_path}")
    return 0
