...
def compute_answer_value_precision_recall_f1(
    prediction_execution: dict[str, Any] | None,
    gold_execution: dict[str, Any] | None,
) -> dict[str, Any]:
    """Compute relaxed answer precision/recall/F1 over executed answers.

    This metric is answer-based, not SPARQL-string-based.

    For SELECT answers, it uses value-only normalization:
    - row order does not matter
    - variable order inside a row does not matter
    - SELECT variable names are ignored
    - returned values, datatypes, and language tags are still compared

    ASK answers are compared like exact binary answers.
    """
